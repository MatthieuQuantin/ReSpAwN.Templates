...
using ProjectName.SharedKernel;
...
public class DomainArchitectureTests : BaseArchitectureTests
{
    ...
    public void Domain_Should_Only_Have_Dependencies_On_Allowed_Dependencies()
    {
        ...
        var result = Types.InAssemblies([DomainAssembly, DomainContractsAssembly])
    ...
    public void Domain_Classes_Should_Be_Sealed_Or_Abstract()
    {
        var result = Types.InAssemblies([DomainAssembly, DomainContractsAssembly])
            ...
            .And().DoNotHaveNameMatching("VogenTypesFactory")// Exclut la factory générée par Vogen
            .Should().BeSealed()
            .Or().BeAbstract()
            .GetResult();
    ...
    [Fact]
    public void Domain_Events_Should_Be_Sealed_And_Reside_In_Events_And_Ending_With_DomainEvent()
    {
        const string EventsNamespace = ".Events";
        const string EventSuffix = "DomainEvent";

        var types = Types.InAssemblies([DomainAssembly, DomainContractsAssembly])
        ...
        foreach (var t in types)
        {
            ...
            if (!t.FullName.StartsWith(DomainAssemblyName, StringComparison.Ordinal) || !t.FullName.Contains($"{EventsNamespace}.", StringComparison.Ordinal))
                issues.Add($"n'est pas dans un namespace {DomainAssemblyName}.*{EventsNamespace}");
        ...
        Assert.True(details.Count == 0, $"{Project} : Les DomainEvents doivent être sealed, dans un namespace valide et bien nommés.{Environment.NewLine}{string.Join(Environment.NewLine, details)}");
    }
}