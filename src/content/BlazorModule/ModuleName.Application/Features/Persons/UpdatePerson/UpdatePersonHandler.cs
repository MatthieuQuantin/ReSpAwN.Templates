    ...
    : ICommandHandler<UpdatePersonCommand, Result>
{
    public async Task<Result> Handle(UpdatePersonCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogError(exception, "Une erreur est survenue lors de la mise à jour de la personne avec l'Id {PersonId}", request.Id);
            return Result.Error($"Une erreur est survenue lors de la mise à jour de la personne avec l'Id {request.Id}");
        }
    }
}