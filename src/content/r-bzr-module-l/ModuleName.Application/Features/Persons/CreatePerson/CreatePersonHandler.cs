    ...
    public async Task<Result<PersonResult>> Handle(CreatePersonCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (personCreateResult.IsInvalid())
                return Result<PersonResult>.Invalid(personCreateResult.ValidationErrors);
        ...
        catch (Exception exception)
        {
            ...
            return Result<PersonResult>.Error($"Une erreur est survenue lors de la création de la personne avec le prénom {request.FirstName} et le nom {request.LastName}");
        }
    }
}