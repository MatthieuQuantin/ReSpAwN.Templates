...
using ModuleName.Domain.PersonAggregate.Events;
    ...
    : ICommandHandler<DeletePersonCommand, Result>
{
    public async Task<Result> Handle(DeletePersonCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var person = await repository.GetByIdAsync(PersonId.From(request.Id), cancellationToken);
            if (person is null)
            {
        ...
        {
            logger.LogError(exception, "Une erreur est survenue lors de la suppression de la personne avec l'Id {PersonId}", request.Id);
            return Result.Error($"Une erreur est survenue lors de la suppression de la personne avec l'Id {request.Id}");
        }
    }
}